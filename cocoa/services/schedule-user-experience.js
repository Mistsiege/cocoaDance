(function() {
	...
	var app = angular.module("scheduleDisplay", ['labDateFilter']);

	var ScheduleController = function($scope, $http, $sce) {
		
		$scope.eventID = "";
		$scope.scheduleTab = "";
		$scope.scheduleDetail = "";
		
		// Set the Schedule Day when the buttons are clicked
		$scope.setScheduleDay = function(eventDay){ 
			$scope.scheduleTab = eventDay;
			$scope.scheduleDetail = "";
		}

		// Set the ID of the detail user wants to see
		$scope.setScheduleDetail = function(detailID){ 
			$scope.scheduleDetail = detailID;
		}
	...
	app.controller("ScheduleController", ["$scope", "$http", "$sce", ScheduleController]);
