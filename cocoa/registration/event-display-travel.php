<?php
	// Lindy and Blues Event Event Display Travel script
	// Version 1.0.1
	...
	// events page so everyone can see what we have coming up.  This script relates specifically to the traveling
	// events (not LaB run).
	//
	// Date		Developer  				Modified
	// -------- ----------------------  --------------------------------------------------------------------------
	// 03-12-2014 Marc Longhenry  		File Created, modified to use the database tables and files for Travel Event Display
	...
	// Retrieve Events with Date From's after today
		$data = mysql_query("SELECT * FROM events WHERE e_time >= CURDATE( ) AND lab_event = 'no' ORDER BY s_time ASC ") or die(mysql_error());
