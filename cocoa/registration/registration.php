<?php
	...
	// 08-01-15		"					Modified to include Car Space and housing updates
	...
	if ($result = $mysqli->query("SELECT * FROM events WHERE event_id = $event_id")) {
		if($row = $result->fetch_assoc()) {
			...
			error_log($error_msg, 3, $root."/backend/registration/_error.log");
		}
		$result->close();
	} else {
	...
	if($vip == 1) { $registration_flag = 'open'; }
	
	
	// Display Registration  -----------------------------------------------------------------------------------
	if($registration_flag == 'open') {
?>
<script src="../../backend/registration/page-specific.js" type='text/javascript'></script>
<div id="event-registration-form" class="event-info-instance">
...
<?php
		if($housing_flag == 'open' || $housing_flag == 'closed') {
?>
		<!-- <div class="registration-section"> -->
			...
			if($housing_end >= date('Y-m-d H:i:s') || $vip == 1) { ?>
...
?>
		<!-- opportunity to do something with housing outside of Open, Closed, or None.  Default removes the housing segment -->
<?php
...
<?php
		...
		echo 'Registration closed at '.date('F jS, Y - g:i A', strtotime($row['reg_end'])).'!!<br /><br />
			<h1>Policy on Refunds for <?php echo $name; ?></h1>
			<p>- You will get a full refund if you cancel your registration before '.date('F jS', strtotime('11 days ago', strtotime($startTime))).'<br />
			- You will get a half refund if you cancel your registration between '.date('F jS', strtotime('10 day ago', strtotime($startTime))).' and '.date('F jS', strtotime('2 days ago', strtotime($startTime))).' (inclusive)<br />
			- There will be no refunds for registrations cancelled on or after '.date('F jS', strtotime('1 day ago', strtotime($startTime))).'</p>
			<h1>If you have any questions, please contact '.$contact_name.' via '.$contact_email.'.</h1>';
	} elseif($registration_flag == 'closed') {
		echo 'Registration is closed for this event!<br /><br />
			<h1>Policy on Refunds for <?php echo $name; ?></h1>
			<p>- You will get a full refund if you cancel your registration before '.date('F jS', strtotime('11 days ago', strtotime($startTime))).'<br />
			- You will get a half refund if you cancel your registration between '.date('F jS', strtotime('10 day ago', strtotime($startTime))).' and '.date('F jS', strtotime('2 days ago', strtotime($startTime))).' (inclusive)<br />
			- There will be no refunds for registrations cancelled on or after '.date('F jS', strtotime('1 day ago', strtotime($startTime))).'</p>
